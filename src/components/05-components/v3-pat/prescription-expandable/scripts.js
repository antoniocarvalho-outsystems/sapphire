...
function PrescriptionExpandableObj() {
	...
	that.ajaxRefresh = function() {
      ...
      $(".PrescriptionExpandable_header").on('click', function(e){
          if($(".PrescriptionExpandable_header").hasClass("NotExpandable")){
              e.preventDefault();
          }
          else{
              clickEvents(this);    
     ...
     if($(".PrescriptionExpandable_header").hasClass("NotExpandable")){
          ...
          else if ($(".PrescriptionExpandable_header").hasClass("isLinkEpandableClick")){
                      $(".PrescriptionExpandable_header__LinksDiv").on('click', function(e){
                           clickEvents(this);
                       });     
          }
      ...
      */
		...
		$('.SectionPrescriptionExpandableArea').each(function() {
			...
			if (curState != previewstat[$(this).attr('id')]['server']) {
				...
				// has class expanded
				if (
					previewstat[$(this).attr('id')]['client'] == false &&
					...
					previewstat[$(this).attr('id')]['client'] == true &&
					!$(this).hasClass('expanded')
				) {
	...
	that.init = function() {
		...
		/*add click events
      $(".PrescriptionExpandable_header").on('click', function(){
          if(!$(this).parent().find('.SectionPrescriptionExpandableArea').hasClass('CursorPointerNone')){
              clickEvents(this);
          }
      });
      $(".PrescriptionExpandable_header").on('click', function(e){
...
// Execute by SilkUI
SilkUI.Execute(
	SilkUI.SectionExpandable.init,
	'Error on WebPatterns/Content/SectionExpandable'
);
